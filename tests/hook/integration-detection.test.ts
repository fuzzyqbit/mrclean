 ...
 *
 * QA-02 tagging (Plan 03-04):
 * Each HOOK-01 event has its own nested describe tagged `@hook-integration` so
 * the CI enforcement step can grep for `@hook-integration.*<EventName>` and
 * mechanically verify that every event in HOOK-01 has at least one passing test.
 */
...
// the developer's real ~/.mrclean. Tests that need a specific HOME still win
// via extraEnv (spread last).
const SANDBOX_HOME = mkdtempSync(join(tmpdir(), 'mrclean-hook-det-home-'))
...
describe('End-to-end hook integration with detection', () => {
  // -------------------------------------------------------------------------
  // QA-02: Integration coverage per HOOK-01 event.
  // Each nested describe is tagged `@hook-integration` so the CI QA-02
  // enforcement grep (`grep "@hook-integration.*<EventName>"`) can
  // mechanically verify that every event in HOOK-01 has at least one passing
  // integration test.
  // -------------------------------------------------------------------------

  describe('@hook-integration UserPromptSubmit', () => {
    ...
    it('Test 5: UserPromptSubmit dry_run=true → no block even for high-risk prompt, audit log populated', () => {
  ...
  describe('@hook-integration PreToolUse', () => {
  ...
  describe('@hook-integration PostToolUse', () => {
  ...
  describe('@hook-integration SessionStart', () => {
