...
- [x] **REVMODE-02**: Operator can enable reversible mode via a `[reversible]` config table (default OFF); an enabled session persists a session-scoped placeholder→original map across hook events as an encrypted file (AES-256-GCM, fresh IV per write, key material in a separate 0700 directory, ciphertext-only on disk including temp files, never inside the project tree)
- [x] **REVMODE-04**: Concurrent hook processes in a reversible session allocate collision-free, stable placeholders — content-addressed allocation under the shared session store (same original → same placeholder across processes), closing the latent per-process counter-collision gap for reversible sessions
- [x] **REVMODE-05**: Reversible sessions issue session-tagged v2 tokens `<MRCLEAN:TYPE:NNN:nonce8>` (per-session CSPRNG nonce, kills planted-token enumeration); the one-way default keeps the shipped v1 placeholder format and code path byte-identical
- [x] **REVMODE-06**: Secret-class originals (all secret TYPEs, ENV, ENTROPY, checksum'd PII such as SSN/credit-card) are never persisted anywhere — map entries for these types structurally lack an `original` field; no configuration can widen this floor, only narrow the restorable set
...
- [x] **REVMODE-11**: CI proves reversible mode never re-exposes originals on any wire path — hook stdout is unchanged by reversible mode, no plaintext canary appears in any written buffer (including atomic-write temp files), chaos tests show redaction survives a corrupt/missing/chmod'd map, and an 8–16-process concurrency stress test gates the build
...
|-------------|-------|--------|
| REVMODE-01 | Phase 10 — Operator Restore | Complete |
| REVMODE-02 | Phase 9 — Session State Adapter | Complete |
| REVMODE-03 | Phase 8 — Contract Verification & Reversible Plumbing | Complete |
| REVMODE-04 | Phase 9 — Session State Adapter | Complete |
| REVMODE-05 | Phase 9 — Session State Adapter | Complete |
| REVMODE-06 | Phase 9 — Session State Adapter | Complete |
| REVMODE-07 | Phase 9 — Session State Adapter | Complete |
| REVMODE-08 | Phase 10 — Operator Restore | Complete |
| REVMODE-09 | Phase 10 — Operator Restore | Complete |
| REVMODE-10 | Phase 8 — Contract Verification & Reversible Plumbing | Complete |
| REVMODE-11 | Phase 11 — Wire-Safety Verification & Hardening | Complete |
