...
- **Test runner:** vitest with a dedicated `tests/perf/*.test.ts` suite. **[discretion-correction per RESEARCH §3]** vitest's `bench()` API does NOT expose `p95` (only p50/p75/p99/p995/p999), and `beforeAll/afterAll` do not fire inside bench suites. Use plain `test()` + `performance.now()` deltas + manual percentile computation — identical pattern to Phase 2's `src/doctor/bench.ts`. Assert with `expect(p95).toBeLessThanOrEqual(100)`. Iterations: N=50 per event.
...
- **Parallel test pollution remediation per RESEARCH §8:** root cause is `globalSetup` running `tsup --clean` which deletes `dist/` mid-run. Fix via vitest `projects` API: integration tests (`tests/install/*`, `tests/doctor/end-to-end.test.ts`, `tests/hook/integration-*`, `tests/fixtures-corpus*`, `tests/perf/*`) go into a `fileParallelism: false` sequential project; unit tests stay in the parallel-fast project. Single `npm test` invocation runs both projects; default invocation passes 359/359 without `--no-file-parallelism`.

### npm Publish (DOC-03)

- **Package name:** `mrclean-claude` per RESEARCH §6 (npm registry check). **[discretion]** Original target `mrclean` is taken (owned by `jackhq` since 2012; abandoned but registered — npm dispute policy doesn't transfer for abandonment). `mrclean-claude` is verified-available, positions the project clearly (Claude Code in-session sanitizer), avoids npm-org setup friction of scoped `@mrclean/mrclean`. Operator can override before the first publish task runs; downstream code paths reference the package name in <10 places.
