# Requirements Archive: v2.0 Native-Node PII/NER Layer
...
- [x] **INST-01**: Operator can run `npx mrclean install` and have a working hook + MCP wiring written into `~/.claude/settings.json` with no further configuration
...
- [x] **HOOK-05**: Hook fails closed — any uncaught exception or invalid input causes exit code 2 with a structured stderr message rather than passing the unsanitized payload through
...
|-------------|-------|--------|
| INST-01 | Phase 1 | Complete (01-02; fail-closed wrapper 01-06 + live UAT 01-07) |
